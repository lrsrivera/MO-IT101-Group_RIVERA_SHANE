...
// Function to disable/enable import buttons
function setImportButtonsState(disabled) {
  const importButtons = document.querySelectorAll('.import-section button');
...
function importEmployeeDetails() {
  ...
  showLoading('Importing employee details...');
...
function importAttendance() {
  ...
  showLoading('Importing attendance records...');
  setImportButtonsState(true);
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const employee = {
        ...
        hourly_rate: parseFloat(document.getElementById('gross_semi_monthly_rate').value) / 160 || 0, // Calculate hourly rate based on 160 hours per month
        username: document.getElementById('employee_id').value,
        password: "password"
      };
      console.log('Employee data to be sent:', employee);
